...
#Main function----------------------------------------------------
function DCFG(; plot_sim = false, verbose = false)
    ...
    KD_ar   = LinRange(Cl_i/Cr_i,Cl_i/Cr_i*0.4,1000)            #Partition coefficient array to calculate distribution coefficient history; KD changes with respect to time;
...
if run_and_plot
    ...
    plot_end = true
    verbose  = false
    x_left, x_right, dx1, dx2, x0, res, Ri, C_left, C_right, C0, Mass, Mass0 = DCFG(; plot_sim = plot_sim, verbose = verbose)
    if plot_end
        #Plotting-------------------------------------------------
        maxC = maximum([maximum(C_left),maximum(C_right)])
        fs = 12.0
