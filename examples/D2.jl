...
function D2(;RefineMethod = 1, plot_sim = false, verbose= false,
    ...
    return x_left, x_right, x0, vec(C_left), vec(C_right), vec(C0),maxC, Tlin, XC_left, XC_right, T, Tstart, Tstop, Tpath, KDlin, KD_sim,T_sim, Mass0, Mass, Mass01, Mass2, C_left_check, C_right_check, T_check, Residual, MB_Error
end
#Run calculation------------------------------------------------------------
...
if run_and_plot
    ...
    x_left, x_right, x0, C_left, C_right, C0, maxC, Tlin, XC_left, XC_right, T, Tstart, Tstop, Tpath, KDlin, KD_sim,T_sim, Mass0, Mass, Mass01, Mass2, C_left_check, C_right_check, T_check,Residual, MB_Error = D2(RefineMethod = 1, plot_sim=plot_sim, verbose=verbose, t_user=t_user, T_user=T_user)
    if plot_end
        ...
        p5 = plot(tpath ./ (60*60*24), Tpath .- 273.0, lw=1.5, color=:black, label=L"\mathrm{Interpolated\ path}",
                  xlabel=L"t\ \mathrm{[days]}", ylabel=L"T\ \mathrm{[°C]}", grid=:on)
        p5 = scatter!(tpath ./ (60*60*24), Tpath .- 273.0, marker=:circle, markersize=4, markercolor=:black,
