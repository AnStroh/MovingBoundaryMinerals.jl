...
#Main function----------------------------------------------------
function DCF(;RefineMethod = 1, plot_sim = false, verbose= false)
    ...
    Ri      = [0.0002       0.0005;]                            #Initial radii [interface position, total length] in [m]
    ...
    KD_ar   = LinRange(1.0,0.7,1000)                            #KD array to calculate distribution coefficient history; KD changes with respect to time;
    ...
    RefineLevel = 7                                             #Refinement level; how many times should the grid be refined
    RefineCond  = 0.001                                       #Refinement condition; refine until last dx on the left side <= RefineCond * Ri[1]
    nPoints     = 40                                            #Number of points for initial grid (h-refinement); It can be a number, if both sides should have the same initial number of points or it can be an array ([left right]), if the resolution for the initial number of points should differ.
...
if run_and_plot
    ...
    plot_end = true
    verbose  = false
    x_left, x_right, dx1, dx2, x0, res, Ri, C_left, C_right, C0 = DCF(RefineMethod = 1, plot_sim=plot_sim, verbose=verbose)
    if plot_end
        #Plotting-------------------------------------------------
        maxC = maximum([maximum(C_left),maximum(C_right)])
        fs = 12.0
