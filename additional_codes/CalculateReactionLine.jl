...
equation of the reaction line of 2 phases from a X-T phase diagram. The results are the 
coefficients a, b, c of X(T) = c + b*T + a*T², which are used in Chemical_Stefan_problem_XT.jl.
=#
using FileIO, DelimitedFiles, Plots

"""
Calculates or assigns the coefficients for two transition lines in a phase diagram. The composition X is a function
    of temperature T. 
    ...
    Coefficients for X(T) = a + b*T + c*T²
"""  
...
#Call function-------------------------------------------------------------
#CSV.file with more points give inaccurate results at the BC (Check ~ X = 1).
#file_name_1 = "examples/Examples_phase_diagram/Lower_Line.csv" 
#file_name_2 = "examples/Examples_phase_diagram/Upper_Line.csv" 

#CSV.file with fewer points give more accurate results at the BC (Check ~ X = 1).
