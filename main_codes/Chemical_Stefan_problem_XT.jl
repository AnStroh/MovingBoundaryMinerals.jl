...
function main(plot_sim,verbose)
    ...
    #Phyics-----------------------------------------------------------------
    Di          = [2.47*1e-6   1*1e-7;]                                                         #Initial diffusion coefficient in [m^2/s] (Spandler & O'Neill (2009), Zhang et al. (2010))); 
    ...
    Tstop       = 1300.0 + 273.15                                                               #End temperature in [K]
    ...
    #for iit = 1:141
        ...
        if any(isnan,L_g)
            ...
            println("L_g has NaNs")
        elseif any(isnan,R_g)
            println("R_g has NaNs")
        elseif any(isinf,L_g)
            println("L_g has Infs")
        elseif any(isinf,R_g)
            println("R_g has Infs")
        end
...
if run_and_plot
    ...
    if plot_end
        ...
        plot(p2,p3,p1,suptitle = L"Thermodynamical\ constrained\ Stefan\ condition", dpi = 300)
    end
end    