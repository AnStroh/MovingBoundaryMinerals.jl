...
Every one of these files — plots, profiles, and the bundled exports alike — carries a small citation line naming the package version and the underlying paper ([Stroh2025](@cite)), so a result stays traceable back to its source even if shared or moved on its own, without being visually intrusive (a thin grey footer on plots, a `#`-comment line in the `.tab` files, a `source` field/cell in `inputs.toml`/`.xlsx`/`.jld2`).
