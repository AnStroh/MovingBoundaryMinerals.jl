...
#Main function----------------------------------------------------
function B6(;RefineMethod = 1, plot_sim = false, verbose= false)
    ...
    Ri      = [1.0      10;]                                    #Initial radii [interface position, total length] in [m]   -> in [L]
...
if run_and_plot
    ...
    verbose   = false
    save_file = false
    x_left, x_right, x0, Ri, Ri0, C_left, C_right, C0, C0_r, KD0, n, maxC = B6(RefineMethod = 1, plot_sim=plot_sim, verbose=verbose)
        if plot_end
