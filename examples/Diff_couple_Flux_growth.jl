...
#Main function----------------------------------------------------
function DCFG(;RefineMethod = 1, plot_sim = false, verbose= false)
    ...
    KD_ar   = LinRange(Cl_i/Cr_i,Cl_i/Cr_i*0.4,1000)            #KD array to calculate distribution coefficient history; KD changes with respect to time;
    ...
    nPoints     = 30                                            #Number of points for initial grid (h-refinement); It can be a number, if both sides should have the same initial number of points or it can be an array ([left right]), if the resolution for the initial number of points should differ.
...
if run_and_plot
    ...
    plot_end = true
    verbose  = false
    x_left, x_right, dx1, dx2, x0, res, Ri, C_left, C_right, C0, Mass, Mass0 = DCFG(RefineMethod = 1, plot_sim=plot_sim, verbose=verbose)
    if plot_end
        #Plotting-------------------------------------------------
        maxC = maximum([maximum(C_left),maximum(C_right)])
        fs = 12.0
