...
function main()
    ...
    #Phyics-------------------------------------------------------
    #Di      = [1.0    1.0]
    Di      = [0.004   0.005;]#Initial diffusion coefficient in [m^2/s]; 
                                    #If you want to calculate D with the Arrhenius equation, set Di = [-1.0 -1.0;]
    D0      = [0.4    0.5;]#Pre-exponential factor in [m^2/s]
    rho     = [1.0      1.0;]       #Normalized densities in [g/mol]
    Ri      = [0.02    1;]      #Initial radii [interface    total length] in [m]
    ...
    KD_ar   = LinRange(1.0,0.5,1000)        #Partition coefficient array to calculate partition coefficient history; KD changes with respect to time;
    ...
    #while t < t_tot
        ...
        #println("dt: $dt, dx1: $dx1, dx2: $dx2, x_right: $(x_right[1]),$(x_right[2])")
        #Update time---------------------------------------------------
        t, dt, it = update_time!(t,dt,it,t_tot)
        #Update time-dependent parameters-------------------------------
        D_l, D_r, KD,T = update_t_dependent_param!(D0,Di,dt,Ea1,Ea2,KD_ar,R,T_ar,t_ar,t,t_tot)
        println("D_l: $D_l, D_r: $D_r, KD: $KD, T: $T")
        ...
        #println("Ri: $Ri, res: $res,")# x_left: $x_left, x_right: $x_right, C_left: $C_left, C_right: $C_right")
        #FEM SOLVER-----------------------------------------------------
        ...
        println("dx1: $dx1, dx2: $dx2", "res: $res","x_left[end]: $(x_left[end]), x_right: $(x_right[1]),$(x_right[2]) ")# "x_left: $x_left", "x_right: $x_right", "C_left: $C_left", "C_right: $C_right")
        #Post-Preprocessing---------------------------------------------
