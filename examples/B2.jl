...
#Main function----------------------------------------------------------
function B2(;RefineMethod = 1, plot_sim = false, verbose= false)
    #-------------------------------------------------------------------
    #If you find a [] with two entries this belong to the respective side of the diffusion couple ([left right])
    #Physics-------------------------------------------------------------
    Di      = [-1           -1;]                        #Initial diffusion coefficient in [m^2/s]               -> not used in this example
    ...
    t_tot   = 2e-3 * Myr2Sec                            #Total time [s]
...
if run_and_plot
    ...
    verbose   = false
    save_file = false
    x_left, x_right, x0, C_left, C_right, C0, t, D, D0, T0, T, Ea, R, Ri, maxC = B2(RefineMethod = 1, plot_sim=plot_sim, verbose=verbose)
    ...
    if plot_end
        #Title: Diffusion couple (flux) - time transformation, sphere
        #Plotting------------------------------------------------------
        fs = 12.0
