...
function DCNI(;RefineMethod = 1, plot_sim = false, verbose= false)
    ...
    nPoints     = 250                                   #Number of points for initial grid (h-refinement); It can be a number, if both sides should have the same initial number of points or it can be an array ([left right]), if the resolution for the initial number of points should differ.
