#= Utilized functions in the framework of the Paper:
    ...
    Version: 1.0
=#
using Diff_Coupled, Diff_Coupled.Benchmarks, Test
...
function main()
    ...
    C     = Cstart * ones(nx,1) #Concentration array in [mol]
    C[end]= Cinf                #Set initial concentration at the first grid point
    ...
    while t < t_tot
        ...
        diag_L_g = diag(L_g)
        ScF      = sum(diag_L_g) / length(diag_L_g)
...
#Testing-----------------------------------------------------------------------
@testset "Simple diffusion sphere (1D)" begin
