...
#Main function----------------------------------------------------
function DCMBG(;RefineMethod = 1, plot_sim = false, verbose= false)
    ...
    Ri      = [0.5    1.0;]                                     #Initial radii [interface position, total length] in [m]   -> in [L]
    ...
    t_tot   = 2e4                                               #Total time [s]                                     -> in [L]/[V]
    n       = 2                                                 #Geometry; 1: planar, 2: cylindrical, 3: spherical
    #History dependent parameters---------------------------------
    KD_ar   = LinRange(Cl_i/Cr_i,Cl_i/Cr_i,1000)                #KD array to calculate distribution coefficient history; KD changes with respect to time;
    ...
    nPoints     = 150                                            #Number of points for initial grid (h-refinement); It can be a number, if both sides should have the same initial number of points or it can be an array ([left right]), if the resolution for the initial number of points should differ.
    ...
    C_right = Cr_i*C_left[end]*ones(res[2],1)*inv(KD_ar[1])     #Initial composition right side in [-]
...
if run_and_plot
    ...
    plot_end = true
    verbose  = false
    x_left, x_right, dx1, dx2, x0, res, Ri, C_left, C_right, C0, maxC = DCMBG(RefineMethod = 1, plot_sim=plot_sim, verbose=verbose)
    if plot_end
        #Plotting-------------------------------------------------
        maxC = maximum([maximum(C_left),maximum(C_right)])
        fs = 12.0
