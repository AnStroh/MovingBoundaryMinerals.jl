...
function DCGMB(;RefineMethod = 1, plot_sim = false, verbose= false)
    ...
    Ri      = [0.2       0.8;]                                      #Initial radii [interface position, total length] in [m]
    Cl_i    = 0.5                                                   #Initial composition left side in [-]
    Cr_i    = 0.7                                                   #Initial composition right side in [-]
    ...
    KD_ar   = LinRange(Cl_i/Cr_i,Cl_i/Cr_i*0.9,1000)                #KD array to calculate distribution coefficient history; KD changes with respect to time;
