...
#Main function-------------------------------------------------
function B1(;RefineMethod = 1, plot_sim = false, verbose= false)
    ...
    Ri      = [0.0005       0.001;]             #Initial radii [interface position, total length] in [m]
    ...
    nPoints     = [20 20]                       #Number of points for initial grid (h-refinement); It can be a number, if both sides should have the same initial number of points or it can be an array ([left right]), if the resolution for the initial number of points should differ.
...
if run_and_plot
    ...
    verbose   = false
    save_file = false
    x_left, x_right, x0, C_left, C_right, C0, t, D, maxC, Ri = B1(RefineMethod = 1, plot_sim=plot_sim, verbose=verbose)
