...
function main(plot_sim,verbose)
    ...
    #Phyics-----------------------------------------------------------------
    Di          = [0.002   0.0004;]                                                         #Initial diffusion coefficient in [m^2/s] (Spandler & O'Neill (2009), Zhang et al. (2010))); 
    ...
    Mass01              = (trapezoidal_integration(x_left.^n*rho[1],C_left)+ trapezoidal_integration(x_right.^n*rho[2],C_right))/Ri[2]    #Initial mass (planar structure)
    #Preallocate variables--------------------------------------------------
    L_g         = spzeros(length(x0),length(x0))                                 #Global LHS matrix   
    Mass        = Float64[]                                                   #Array to store the mass of the system
    Mass2       = Float64[]                                                   #Array to store the mass of the system
    ...
    #for iit = 1:1
        ...
        for iit in enumerate(1)       
            ...
            push!(Mass2, Massnow2)                                                            #Stores the mass of the system
    ...
    #println("The concentration differenc for the left phase is $(ErrCL*100)%.")
