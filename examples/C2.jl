...
#Main function----------------------------------------------------
function C2(;RefineMethod = 1, plot_sim = false, verbose= false)
    ...
    RefineLevel = 1                                             #Refinement level; how many times should the grid be refined
    RefineCond  = 0.01                                          #Refinement condition; refine until last dx on the left side <= RefineCond * Ri[1]
    nPoints     = 75                                            #Number of points for initial grid (h-refinement); It can be a number, if both sides should have the same initial number of points or it can be an array ([left right]), if the resolution for the initial number of points should differ.
...
if run_and_plot
    ...
    verbose   = false
    save_file = false
    x_left, x_right, x0, Ri, Ri0, C_left, C_right, C0, C0_r, n, maxC, KD0 = C2(RefineMethod = 1, plot_sim=plot_sim, verbose=verbose)
    if plot_end
